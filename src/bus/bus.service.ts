...
export class BusService {
  ...
  async create(createBusDto: CreateBusDto) {
    ...
    const existingBus = await this.findBusByMatriculation(
      companyUuid,
      matriculation,
    );
  ...
  async findAll(params: PaginateRequest) {
    ...
    let query = await this.busRepository.createQueryBuilder('bus');
    if (params.matricule) {
      query = query.where('bus.matriculation = :matriculation', {
        matriculation: params.matricule,
      });
    }
  ...
  async update(id: string, updateBusDto: UpdateBusDto) {
    const { typeOfBus, companyUuid, matriculation, numberOfSeats } =
      updateBusDto;
    //find existing us
  ...
  async findBusByMatriculation(companyUuid: string, matricule: string) {
    return await this.busRepository
      .createQueryBuilder('bus')
      .where('bus.companyId = :companyId', { companyId: companyUuid })
      .andWhere('bus.matriculation = :matriculation', {
