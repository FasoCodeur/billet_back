...
export class UserService {
  ...
  async forgetPassword(forgetPassWordDto: ForgetPassWordDto) {
    const { newPassword, emailVerificationCode, email, updatedAt } =
      forgetPassWordDto;
    ...
    if (!user.verificationCodes) {
      throw new BadRequestException(
        "Cet utilisateur ne dispose pas d'un code verification, veuillez demamder un autre.",
      );
    }
    const verificationCode = await this.verifyCodeRepository.findOne({
      where: { id: user.verificationCodes.id },
    });
    console.log(verificationCode);
    if (
      verificationCode.emailVerificationCode &&
      verificationCode.emailVerificationCode == parseInt(emailVerificationCode)
    ) {
      if (
        !isExpired(verificationCode.emailVerificationCodeExpiry) &&
        ...
        throw new BadRequestException(
          'Veuillez demander un nouveau code de vérification, celui-ci est déjà expiré.',
        );
      }
    } else {
      throw new BadRequestException(
        'votre code de verification est incorrecte',
      );
    }
  }
  ...
  async createEmailVerificationCode(
    ...
    const user = await this.findUserByEmail(email);
    if (user.verificationCodes) {
      const verificationCodes = await this.verifyCodeRepository.findOne({
        where: { id: user.verificationCodes.id },
      });
    ...
    verificationCode.emailVerificationCodeExpiry =
      param.emailVerificationCodeExpiry;
  ...
  async verifyEmailCode(code: number, email: string) {
    ...
    if (user.verificationCodes === null) {
      throw new BadRequestException(
        "Pour poursuivre, veuillez demander un code de vérification, car celui-ci n'a pas été généré pour cet utilisateur.",
      );
    }
    const verificationCode = await this.verifyCodeRepository.findOne({
      where: { id: user.verificationCodes.id },
